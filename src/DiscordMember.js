...
class DiscordMember {
  ...
  async verify (options) {
    ...
    if (this.user.bot) {
      return status({
        status: false,
        error: ':robot: RCDVer cannot verify bots.'
      })
    }

    // Ignore users with this specific role (to give server owners more power)
    if (this.member.roles.find(role => role.name === 'RCDVer Bypass')) {
      return status({
        status: false,
        error: ':octagonal_sign: RCDVer cannot act on users with the "RCDVer Bypass" role.',
        nonFatal: true
      })
    }

    // Create a warning to append to any errors. In some permission setups, RCDVer is reliant on role positioning (specifically if it has administrator or not)
    if (!(await this.server.members.fetch(this.bot.user.id)).manageable) {
      errorAppend = "\n\nRCDVer's position in the role list is below that of this user. Please have a server admin drag RCDVer's role above all other roles in order to fix this problem."
    }
    ...
    if (data.status === 'ok') {
        ...
        if (!this.member.roles.has(role) && this.server.roles.has(role)) {
          try {
            ...
            return status({
              status: false,
              nonFatal: true,
              error: "RCDVer doesn't have permissions to add roles to that user." + errorAppend
            })
          }
        }
      }
        ...
        if (this.member.roles.has(role) && this.server.roles.has(role)) {
          try {
            ...
            return status({
              status: false,
              nonFatal: true,
              error: "RCDVer doesn't have permission to remove roles from that user." + errorAppend
            })
          }
        }
      }
      ...
      if (
        ...
        !this.member.roles.find(role => role.name === 'RCDVer Nickname Bypass')
      ) {
        ...
        if (this.member.displayName !== nickname) {
          try {
            ...
            return status({
              status: false,
              nonFatal: true,
              error: this.member.guild.ownerID === this.member.id ? "Sorry, Discord doesn't allow bots to change the server owner's nickname. Please manually update your nickname. Or don't, I'm just an error message." : "RCDVer doesn't have permission to change that user's nickname." + errorAppend
            })
          }
        }
      }
